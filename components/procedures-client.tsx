      ...
      >
        ...
        {canEdit && (
          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
            {isOperationLoading && <Spinner size="sm" className="mr-2" />}
            {!isArchived && (
              <>
                <SortButtons
                  sortType={sortType}
          ...
          action={async (formData) => {
            ...
            try {
              ...
              if (title) {
                ...
                const result = await createProcedure(newFormData) as ProcedureResponse
                if (result.success && result.procedure) {
                  await onProcedureCreated(categoryId, result.procedure)
                  setIsOpen(false)
                }
              }
            } catch (error) {
